...
class TipoVehiculo():    
    def tipoVehiculo(x):
            t_vehiculo = 'OTROS'
            if x in (['AUTOS DE NEGOCIOS, TAXIS Y MICROBUSES URBANOS','AUTOS FAMILIARES']):
                t_vehiculo = 'AUTOS'
            if x in (['MOTOS']):
                t_vehiculo = 'MOTOS'
            if x in(['CAMPEROS O CAMIONETAS']):
                t_vehiculo = 'CAMPEROS'
            if x in(['VEHICULOS DE SERVICIO PUBLICO INTERMUNICIPAL']):
                t_vehiculo = 'INTERMUNICIPAL'
            return(t_vehiculo) 
...
#            BASE_MODELO_5['INTERPRETABILIDAD_SCORE'])
            ...
            cursor.execute("""DELETE FROM FRAUD.TBL_MN_PFRAU_PREDICCIONES_HIS WHERE FECHA_PROCESO =  (SELECT MAX(FECHA_PROCESO) FECHA_PROCESO FROM STAGE.STG_MN_PFRAU_PREDICCIONES_V)
            ...
            filePredic2['Glosas']=np.where(filePredic2['SCORE_PREDICCION'].astype(float)>=float(valor_corte.replace(',','.')),1,0)
